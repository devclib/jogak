<script lang="ts">
  ...
  let {
    ...
    size = 'sm',
    outline = false,
  }: { label: string; tone?: Tone; size?: Size; outline?: boolean } = $props()
...
<span
  class="inline-flex items-center rounded-full font-medium tracking-wide uppercase {size === 'sm' ? 'text-[10px] px-1.5 py-0.5' : 'text-xs px-2 py-1'} {outline ? `border ${outlineClass[tone]} bg-white` : toneClass[tone]}"
>
  {label}
</span>
